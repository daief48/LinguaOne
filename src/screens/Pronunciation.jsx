...
import { pronunciationWord as pw } from '../data/mock'
import { tint as getTint, scoreTint } from '../data/tints'

function ScoreResult({ onRetry }) {
  const score = useCountUp(pw.score, { duration: 1100 })
  const t = getTint(scoreTint(pw.score))

  return (
    <div className="animate-slide-up">
      <div className="surface overflow-hidden p-0">
        <div className={`flex items-center gap-4 bg-gradient-to-br ${t.gradSoft} p-4`}>
          <CircularProgress value={pw.score} size={92} stroke={9} tint={scoreTint(pw.score)} label={`${score}%`} />
          <div className="min-w-0">
            <div className="flex items-center gap-1.5">
              <Check size={15} className="text-emerald-600" strokeWidth={3} />
              <h3 className="font-display text-[17px] font-extrabold text-ink-900">{pw.verdict}</h3>
            </div>
            <p className="mt-1 text-[12.5px] leading-relaxed text-ink-500">
              Very close to a native speaker. One vowel needs a little more length.
            </p>
          </div>
        </div>
      ...
      <div className="mt-4 space-y-2.5">
        ...
        <Button to="/practice" variant="neutral" size="lg" iconRight={ArrowRight}>
          Next word
        </Button>
      </div>
    </div>
...
export default function Pronunciation() {
  ...
  return (
    <div className="flex-1 pb-8">
      <TopBar title="Pronunciation Coach" subtitle="Word practice" backTo="/home" />

      <div className="px-4 pt-3">
        {/* word card */}
        <div className="relative overflow-hidden rounded-4xl border border-pink-100 bg-gradient-to-br from-pink-50 via-white to-violet-50/60 p-5 text-center shadow-soft">
          ...
          <p className="relative mt-2 font-mono text-[15px] font-medium text-ink-500">{pw.phonetic}</p>

          {/* syllables */}
          <div className="relative mt-4 flex items-center justify-center gap-1.5">
            {pw.syllables.map((s, i) => (
              <span
                key={i}
          ...
          <p className="relative mt-3 text-[13px] leading-relaxed text-ink-500">{pw.meaning}</p>
        </div>
        ...
        <div className="mt-4 surface p-4">
          <div className="flex items-center gap-2.5">
            ...
            <button
              type="button"
              onClick={() => showToast(`Playing “${pw.word}”`, { variant: 'ai' })}
          ...
          <div className="mt-3 rounded-2xl bg-ink-50 p-3">
            <Waveform bars={30} active={false} tint="violet" height={32} seed={2} />
          </div>
        </div>
          ...
          <div className="flex flex-wrap gap-2">
            {pw.nextWords.map((w) => (
              <button
                key={w}
                type="button"
                onClick={() => showToast(`“${w}” queued for practice`, { variant: 'default' })}
                className="press focus-ring flex items-center gap-2 rounded-2xl border border-ink-100 bg-white px-3 py-2.5 shadow-soft hover:border-ink-200"
              >
                <span className="grid h-7 w-7 place-items-center rounded-lg bg-pink-50 text-pink-600">
                  <Mic size={13} strokeWidth={2.5} />
                </span>
                <span className="text-[13px] font-bold text-ink-800">{w}</span>
              </button>
            ))}
          </div>
        </div>
